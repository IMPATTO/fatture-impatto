...
const AUTOMATION_ID = 'extra-10-oggi-16';
...
function renderMarkdown(report) {
  ...
  if (!report.applied.length) {
    ...
    for (const item of report.applied) {
      lines.push(`- ${item.apartmentName} / ${item.unitLabel} / ${item.date}: ${item.currentPrice} -> ${item.newPrice}`);
    }
  }
...
async function loadContext(client, todayIso) {
  const [
    ...
    client
      ...
      .select('apartment_id,room_id,date_from,date_to,status,created_by,write_payload')
...
async function main() {
  ...
  if (args.help) {
    console.log('Usage: node scripts/extra-10-oggi-16.mjs [--today=YYYY-MM-DD] [--dry-run=true|false] [--format=json|markdown]');
    process.exit(0);
  }
